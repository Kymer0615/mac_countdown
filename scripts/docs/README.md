...
- Captures only the demo status item, its dropdown, and the events and settings windows. Widget images render the actual SwiftUI content with a dark rounded background; they are previews rather than screenshots of desktop widget placement.
- Generates the accelerated start-to-critical-window-to-deadline GIF with `MoonIcon`, `MoonProgress`, and `CountdownFormat`, and includes a still PNG alternative. The final frame uses the app's completion checkmark.

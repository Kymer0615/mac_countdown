...
- Captures only the demo status item and its dropdown. Widget images render the actual SwiftUI content with a dark rounded background; they are previews rather than screenshots of desktop widget placement.
- Generates the accelerated GIF with `MoonIcon`, `MoonProgress`, and `CountdownFormat`, and includes a still PNG alternative. The final frame uses the app's completion checkmark.
